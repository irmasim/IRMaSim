...
class Energy2(WM):
    def __init__(self, simulator: 'Simulator'):
        super(Energy2, self).__init__(simulator)
    ...
    def layout_job(self, job: Job):
        ...
        for node in viable_nodes:
            node_ener = self.node_sort_key(node, job)
            print(node_ener, min_ener)
            if node_ener < min_ener:
                min_ener = node_ener
        ...
        if selected_nodes:
            self.assigned_nodes[selected_nodes[-1].id] += 1
