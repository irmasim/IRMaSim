...
class BackfillV1_1(WorkloadManager):
    def __init__(self, simulator: 'Simulator'):
        super(BackfillV1_1, self).__init__(simulator)
        if simulator.platform.config["model"] != "modelV1_1":
            raise Exception("BackfillV1_1 workload manager needs a modelV1_1 platform")
        options = Options().get()
    ...
    def schedule_next_job(self):
        ...
        if len(self.pending_jobs) == 0 or len(self.nodes) == 0:
            return False
