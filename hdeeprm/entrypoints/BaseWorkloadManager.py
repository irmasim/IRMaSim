...
Attributes:
    ...
    core_changes (dict):
        History of recent changes in p-states of cores
    """
...
consumption.
        ...
        for pstate in range(number_p_states+1):
            ...
            if resources:
                ...
                for i in list(resources):
                    if i not in self.core_changes:
                        self.core_changes[i] = ChangesCores(i, self.bs.time(),
                                               self.resource_manager.core_pool[i].state['current_power'])
                    else:
                        self.core_changes[i].add_change(self.bs.time(),
                                                        self.resource_manager.core_pool[i].state['current_power'])

                #logging
                logging.debug("Resource " + str(ProcSet(*resources))
                              +  " " + str(pstate)+ "  Time " + str(self.bs.time()))
